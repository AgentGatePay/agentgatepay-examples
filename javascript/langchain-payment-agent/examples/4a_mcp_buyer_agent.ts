 ...
 * Run the seller agent FIRST (4b_mcp_seller_agent.ts), then run this buyer.
 *
 * The buyer agent:
 ...
 *     npm run example:4b  # In another terminal
 *
 *     # Then run buyer
 *     npm run example:4a
 *
...
// See Example 3b (examples/3b_mcp_with_tx_service.ts) for external signing usage.
//
// ========================================
...
Think step by step and complete the workflow for ONE resource.`;
  ...
  try {
    ...
    console.error(`   Please start the seller first: npm run example:4b`);
    process.exit(1);
  }
